...
if [ "$1" = "--frontend" ]; then
    ...
    docker compose -p ${PROJECT_NAME} -f docker-compose.prod.yml --env-file .env restart frontend
    echo -e "${GREEN}✅ Frontend updated!${NC}"

# Option 2: Restart backend
elif [ "$1" = "--backend" ]; then
    echo -e "${YELLOW}Restarting backend only...${NC}"
    docker compose -p ${PROJECT_NAME} -f docker-compose.prod.yml --env-file .env restart backend
    echo -e "${GREEN}✅ Backend updated!${NC}"

# Option 3: Restart both (default)
else
    echo -e "${YELLOW}Restarting all services...${NC}"
    docker compose -p ${PROJECT_NAME} -f docker-compose.prod.yml --env-file .env restart
    echo -e "${GREEN}✅ All services updated!${NC}"
fi

echo
echo -e "${BLUE}Note: Frontend runs in dev mode - changes appear instantly after restart!${NC}"
echo -e "${YELLOW}View logs: docker-compose -p ${PROJECT_NAME} -f docker-compose.prod.yml --env-file .env logs -f${NC}"